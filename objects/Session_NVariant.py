...
paragraph_patterns = 'paragraph\s(?:\d|one|two|three|for|four|five|six|seven|eight|nine|ten|to)'
chatgpt_patterns = 'please\s.*'
confirm_request_patterns = 'send request'
cancel_request_patterns = 'cancel request'

class Session_NVariant(Session):
    ...
    def _new_line(self):
        if self.paragraphs[self.index] != ['']:
            ...
            self.commands.append(f'Added New Paragraph based on command {self.input_text}')
    ...
    def _chatgpt(self):
        ...
        self.commands.append(f'Creating chatgpt request from command: "{self.input_text}"')
        logging.info(self.commands[-1])
    ...
    def _confirm_request(self):
        if self.chatgpt != '':
            self.chatgpt = ''
            self.commands.append(f'Received command to send request: "{self.input_text}"')
            logging.info(self.commands[-1])

    def _cancel_request(self):
        if self.chatgpt != '':
            self.chatgpt = ''
            self.commands.append(f'Received command to cancel request: "{self.input_text}"')
            logging.info(self.commands[-1])
