...
class HTML_Interface:
    ...
    def html_text(update):
        ...
        for i, paragraph in enumerate(update.get('paragraphs')):
            if i != update.get('index'):
                ...
                if len(paragraph)>1:
                    output+=f'<span>'+' '.join(paragraph[:-1])+'</span>'+f'<span style="color:{active_sentence_colour};">'+' '+paragraph[-1]+'</span></mark><br><br>'
                elif len(paragraph)==1:
                    output+=f'<span style="color: {active_sentence_colour};">'+paragraph[-1]+'</span></mark><br><br>'
        return output

    @staticmethod
    def html_commands(update):
        output = '<ul>'+''.join(['<li>'+x+'</li>' for x in update.get('commands')[::-1]])+'</ul>'
        return output
    ...
    def html_chatgpt(update):
        ...
        if request!='':
            ...
            output += '<span> as a chatgpt request for the following text: </span>'
            output +=f'<mark style="background: {active_paragraph_colour}!important">'
